...
class TicketViewTest(TestCase):
    def setUp(self):
        ...
        self.manager_role.permissions.add(
            self.edit_permission, self.status_permission, self.comment_permission
        )
        self.analyst_role.permissions.add(
            self.status_permission, self.comment_permission
        )
    ...
    def test_ticket_create_view(self):
        ...
        response = self.client.post(
            reverse("ticket_add"), {"name": "New Ticket", "status": self.status.id}
        )
    ...
    def test_ticket_edit_view(self):
        self.client.login(email="admin@example.com", password="password123")
        response = self.client.post(
            reverse("ticket_edit", args=[self.ticket.id]),
    ...
    def test_ticket_update_status_view(self):
        ...
        response = self.client.post(
            reverse("ticket_update_status", args=[self.ticket.id]),
    ...
    def test_comment_create_view(self):
        self.client.login(email="analyst@example.com", password="password123")
        response = self.client.post(
            reverse("ticket_detail", args=[self.ticket.id]), {"text": "Test comment"}
        )
        self.assertEqual(response.status_code, 302)
